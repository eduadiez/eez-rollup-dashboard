...
// hasClaimed(address) on the current FlashLoanersNFT.
...
async function loadClaimInfo(
  ...
  try {
    ...
    if (claimL2Block !== null) {
      ...
      // The current topology exposes one L2 RPC. Keep the legacy fullnode
      // comparison empty rather than comparing the endpoint with itself.
      fullnodeRoot = null;
    }
...
export function useFlashLoan(
  ...
  useEffect(() => {
    ...
    async function checkDeployment(): Promise<boolean> {
      ...
      try {
        ...
        // The current NFT records the L2 executor in hasClaimed(address).
        if (deployed && nftAddress && nftAddress !== ZERO_ADDR && executorL2) {
          try {
            const nftResult = (await rpcCall(config.l2Rpc, "eth_call", [
              { to: nftAddress, data: HAS_CLAIMED_SELECTOR + pad32(executorL2) },
              "latest",
            ])) as string;
            const claimed = decodeUint256(nftResult) > 0n;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const executorL2 = stateRef.current.executorL2;
        if (!executorL2) return;
        const nftResult = (await rpcCall(config.l2Rpc, "eth_call", [
          { to: nftAddress, data: HAS_CLAIMED_SELECTOR + pad32(executorL2) },
          "latest",
        ])) as string;
  ...
  const execute = useCallback(async () => {
    ...
    if (alreadyClaimed) {
      log("Flash loan already executed — this L2 executor already claimed its NFT.", "info");
      return;
    }
    ...
    // Also check the current NFT claim state on L2.
